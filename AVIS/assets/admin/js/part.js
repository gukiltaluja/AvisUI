  ...
  var count = 0;  
   debugger
      for (var i = 0; i < data.length; i++) {
        var edit =  '<a onclick="findPartById('+data[i].id+')" ><i class="fa fa-edit" ></i></a>';
    count++;
    table.row.add([
        count,
        data[i].partTypeName,
...
function findPartById(id){
        debugger
          tyresdata = callajaxByUrlVarData(getPartById,id);
          tyre = tyresdata.data;
          console.log(id);
          console.log(tyre);
          debugger
          //alert(tyre);
          //alert(id);
          $('#partType').val(tyre.partTypeId);
          $('#partName').val(tyre.partName);
           $('#partPrice').val(tyre.price);
           $('#partId').val(tyre.id);
        }

function partUpdate(){
        var a = document.getElementById("partName").value;
       if (a == "") {
          swal({
              title: 'Failed',
              text: 'Please Enter Part Name',
          ...
          return;
              ...
              var json={
                  "id":$("#partId").val(),
                  "partName": $("#partName").val(),
                  "partTypeId": parseInt($("#partType").val()),
                  "price": parseInt($("#partPrice").val())
              }
