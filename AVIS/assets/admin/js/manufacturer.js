  ...
  var count = 0;  
   debugger
      for (var i = 0; i < data.length; i++) {
        ...
        var edit =  '<a onclick="findmanufacturerById('+data[i].id+')" ><i class="fa fa-edit" ></i></a>';
    count++;
    table.row.add([
        count,
...
function findmanufacturerById(id){
        debugger
          tyresdata = callajaxByUrlVarData(getManufacturerById,id);
          tyre = tyresdata.data;
          console.log(id);
          console.log(tyre);
          debugger
          $('#manufacturerName').val(tyre.manufacturerName);
          $('#manufacturerAddress').val(tyre.manufacturerAddress);
           $('#manufacturerId').val(tyre.id);
        }
