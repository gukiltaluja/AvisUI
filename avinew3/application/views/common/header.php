...
<header>
    ...
    <div class="nav">
       ...
       <div class="searchFull">
        <input name="q" type="text" class="input w70 vam" id="req_text_Keyword" value="Type Keywords here. . ." onFocus="if(this.value=='Type Keywords here. . .')this.value=''" onBlur="if(this.value=='')this.value='Type Keywords here. . .'">
        <input type="image" src="<?php echo ASSET_URL?>images/spacer.png" class="cp vam button" alt="search">
        <span id="srchspan"></span>
    </div>
     <script>
        $(document).ready(function() { 
			...
			/*$('#srchspan').click(function(){
				window.location.href= '/search/'+$('#req_text_Keyword').val();
			})*/
			
			$('#req_text_Keyword').on('keypress',function(event){
				if(event.which=='13'){
					var value=$("#req_text_Keyword").val();value=value.replace(/ /g,"-");value=value.replace(/</g,"");value=value.replace(/>/g,"");window.location.href="/search/"+escape(value);
					}
				});
...
if(($controller == 'products_guide' && $method == 'index') OR ($controller == 'search' && $method == 'index')){
